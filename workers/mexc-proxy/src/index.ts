...
async function handleTelegram(
  ...
  if (
    ...
    const memeText = [
      ...
      '• SHORT veto: dominating bid supply · OBI ≥10% · bid/ask ≥1.55 · forecast NEXT_UP',
      '• LONG veto: dominating ask supply · OBI ≤−10% · bid/ask ≤0.65 · forecast NEXT_DOWN',
...
async function maybeAnnounceEngine(env: Env): Promise<void> {
  if (botLane(env) !== 'elite') {
    await announceEngineToChannel(env, 'meme', BOT_ENGINE, [
      ...
      '• Стены: вето только если opposite supply доминирует, не любой уровень $1200.',
      '• LONG/SHORT: phase + BTC + momentum + свечи, затем 3 снимка стакана.',
