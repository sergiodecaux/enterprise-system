...
async function handleTelegram(
  ...
  if (
    ...
    const memeText = [
      ...
      targetBlock,
      '',
      `<b>Обновление v28.3:</b>`,
      '• Только Jeweler Burst; legacy и внешний Jeweler Live отключены',
      '• Каждый скан: 6 сильных movers + 6 ликвидных sideways-монет',
      '• Sideways остаются в sticky-списке, даже если 24h &lt; 4%',
      '• RANGE: обе стороны номинируются в коробке, стакан/лента выбирают направление',
      '• Граница RANGE: reclaim/reject при sync 8 + forecast; mid-range — sync 15 и score ≥75',
      '• 6 live books, 3 слота зарезервированы под RANGE',
      '• Если LONG и SHORT оба прошли — нужен зазор quality ≥4',
...
async function maybeAnnounceEngine(env: Env): Promise<void> {
  if (botLane(env) !== 'elite') {
    await announceEngineToChannel(env, 'meme', BOT_ENGINE, [
      ...
      '• Каждый скан резервирует 6 мест для ликвидных sideways-монет; они не выпадают из sticky.',
      '• Ищет PEAK и RANGE: отбой от границы или подтверждённый пробой.',
      '• В RANGE обе стороны номинируются; стакан выбирает LONG/SHORT.',
      '• Граница: sync ≥8 + forecast. Mid-range: sync 15 и direction score ≥75.',
      '• 6 live books, 3 слота под RANGE; LONG vs SHORT нужен зазор quality ≥4.',
      '• LONG/SHORT: phase + BTC + momentum + свечи, затем 3 снимка стакана.',
